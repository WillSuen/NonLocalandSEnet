...
def main():
    if args.data_type == "cifar10":
        ...
        ## add non local resnet
        if args.non_local:
            symbol = nonlocal_resnet(units=units, num_stage=3, filter_list=filter_list, num_class=args.num_classes,
        ...
        if args.non_local:
            symbol = nonlocal_resnet(units=units, num_stage=3, filter_list=filter_list, num_class=args.num_classes,
                            data_type="cifar100", resample=args.resample, bottle_neck = bottle_neck, 
                            bn_mom=args.bn_mom, workspace=args.workspace, memonger=args.memonger)
        else:
            symbol = resnet(units=units, num_stage=3, filter_list=filter_list, num_class=args.num_classes,
                                data_type="cifar100", bottle_neck = bottle_neck, bn_mom=args.bn_mom, workspace=args.workspace,
                                memonger=args.memonger)
    elif args.data_type == "tiny-imagenet":
        args.num_classes = 12
        if args.depth == 18:
            units = [2, 2, 2, 2]
        elif args.depth == 34:
    ...
    train = mx.io.ImageRecordIter(
        path_imgrec         = os.path.join(args.data_dir, "cifar10_train.rec") if args.data_type == 'cifar10' else
                              os.path.join(args.data_dir, "cifar100_train.rec") if args.data_type == 'cifar100' else
                              os.path.join(args.data_dir, "tiny-imagenet-10_train.rec") if args.data_type == 'tiny-imagenet' else
                              os.path.join(args.data_dir, "train_256_q90.rec") if args.aug_level == 1
                              else os.path.join(args.data_dir, "train_480_q90.rec"),
        label_width         = 1,
        data_name           = 'data',
        label_name          = 'softmax_label',
        data_shape          = (3, 32, 32) if args.data_type=="cifar10" or args.data_type=="cifar100" else (3, 224, 224),
        batch_size          = args.batch_size,
        pad                 = 4 if args.data_type == "cifar10" or args.data_type == "cifar100" else 0,
        fill_value          = 127,  # only used when pad is valid
        rand_crop           = True,
        max_random_scale    = 1.0,  # 480 with imagnet, 32 with cifar10
        min_random_scale    = 1.0 if args.data_type == "cifar10"  or args.data_type == "cifar100" 
                              else 1.0 if args.aug_level == 1 else 0.533,  # 256.0/480.0
        max_aspect_ratio    = 0 if args.data_type == "cifar10" or args.data_type == "cifar100" 
                              else 0 if args.aug_level == 1 else 0.25,
        random_h            = 0 if args.data_type == "cifar10" or args.data_type == "cifar100" 
                              else 0 if args.aug_level == 1 else 36,  # 0.4*90
        random_s            = 0 if args.data_type == "cifar10" or args.data_type == "cifar100"
                              else 0 if args.aug_level == 1 else 50,  # 0.4*127
        random_l            = 0 if args.data_type == "cifar10" or args.data_type == "cifar100" 
                              else 0 if args.aug_level == 1 else 50,  # 0.4*127
    ...
    val = mx.io.ImageRecordIter(
        path_imgrec         = os.path.join(args.data_dir, "cifar10_val.rec") if args.data_type == 'cifar10' else 
                              os.path.join(args.data_dir, "cifar100_val.rec") if args.data_type == 'cifar100' else
                              os.path.join(args.data_dir, "tiny-imagenet-10_val.rec") if args.data_type == 'tiny-imagenet' else
    ...
    model = mx.model.FeedForward(
        ...
        num_epoch           = 200 if args.data_type == "cifar10"  or args.data_type == "cifar100" else 120,
        ...
        lr_scheduler        = multi_factor_scheduler(begin_epoch, epoch_size, step=[120, 160], factor=0.1)
                             if args.data_type=='cifar10' or args.data_type=='cifar100' else
    ...
    model.fit(
        X                  = train,
        eval_data          = val,
        eval_metric        = ['acc', 'ce'] if args.data_type=='cifar10' or args.data_type == "cifar100" else
...
if __name__ == "__main__":
    ...
    parser.add_argument('--non_local', action='store_true', default=False, help='true means add non local nets')
    parser.add_argument('--resample', action='store_true', default=False, help='true means add resample layer')
